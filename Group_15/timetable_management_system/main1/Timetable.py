...
from main1 import Settings, TableModel
import json
...
class Timetable:
    def __init__(self, table, data=False):
        ...
        table.clicked.connect(self.toggling)
        table.horizontalHeader().sectionClicked.connect(self.toggling)
        table.verticalHeader().sectionClicked.connect(self.toggling)
        table.findChild(QtWidgets.QAbstractButton).clicked.connect(self.toggling)
